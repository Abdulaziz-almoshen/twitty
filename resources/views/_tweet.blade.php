<div class=" pb-2 border border-gray-200 rounded-lg ">
    <div class="flex p-3 ">
        ...
        <div>
            <strong>
                <h2><a href="{{route('profile',$tweet->user->name)}}">{{$tweet->user->name}}</a></h2></strong>
            ...
            <p class="text-md">
                ...
                <div class="flex pt-2">
                    <div class="px-3 flex">
                        <form action="{{route('likeTweet', $tweet)}}" METHOD="post">
                            @csrf
                            <button TYPE="submit" >
                                @if($tweet->islike(auth()->user()))
                                <svg viewBox="0 0 20 20" class="text-gray-600 w-4">
                                    @else
                                    <svg viewBox="0 0 20 20" class="text-gray-400 w-4">
                                        @endif
                                        <g id="Page-1" stroke="none" stroke-width="1" fill="none"
                                           fill-rule="evenodd">
                                            <g class="fill-current px-3 ">
                                                <path
                            ...
                            <p class="text-xs pr-1">{{$tweet->likesCount()}}</p>
                    </div>
                    <div class="flex">
                        <form action="{{route('dislikeTweet', $tweet)}}" METHOD="post">
                            @csrf
                            <button TYPE="submit" >
                                    @if($tweet->isDislike(auth()->user()))
                                        <svg viewBox="0 0 20 20" class="text-gray-600 w-4">
                                            @else
                                                <svg viewBox="0 0 20 20" class="text-gray-400 w-4">
                                                    @endif
                     ...
                     <p class="text-xs px-2">{{$tweet->dislikesCount()}}</p>
                     </div>
                </div>
        </div>
    </div>
